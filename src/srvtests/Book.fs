...
[<Tests>]
let bookTests =
  let book = { id = Some <| Id "stringid"; name = "notemptystr"; startDate = DateTime.Now }

  testList "Book should" [
    test "create new" {
      let cmd = OpenBook book
      let event = execute Book.zero cmd
      Expect.equal event (BookOpened book |> Ok) "Expecting open book event"
    }

    test "validate book id when create new" {
      let cmd = OpenBook { book with id = None }
      let actual = execute Book.zero cmd
      Should.Expect.isErrorOf <@ InvalidBookId @> actual "Expecting book cannot be created because of invalid id"
    }
